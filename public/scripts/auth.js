...
function goToDashboard() {
  document.getElementById(
    "index-container"
  ).innerHTML = `<h2>Welcome to ADJC Bank!</h2>
...
async function signup(event) {
  ...
  if (!(usernameValid && passwordValid && balanceValid)) {
    showToast(
      "Please follow the correct format to enter your information",
...
async function login(event) {
  ...
  try {
    ...
    if (querySnapshot.empty) {
      showToast(
        `Cannot find your Username : ${username}, please Sign-up! Redirecting... `,
        "alert-toast"
      );
      setTimeout(() => {
        toSignUpHTML;
    ...
    querySnapshot.forEach((doc) => {
      ...
      localStorage.setItem(
        "currentUser",
        JSON.stringify({ id: doc.id, data: userData })
      );
    });
